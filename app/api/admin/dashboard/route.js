...
export async function GET(req) {
  ...
  const [
    ...
    prisma.orderItem.aggregate({ where: { transaction: { createdAt: { gte: today, lt: tomorrow }, status: 'COMPLETED' } }, _sum: { subtotal: true }, _count: true }),
    prisma.orderItem.aggregate({ where: { transaction: { createdAt: { gte: thisMonthStart }, status: 'COMPLETED' } }, _sum: { subtotal: true }, _count: true }),
    prisma.orderItem.aggregate({ where: { transaction: { createdAt: { gte: lastMonth, lte: lastMonthEnd }, status: 'COMPLETED' } }, _sum: { subtotal: true }, _count: true }),
    ...
    prisma.orderItem.findMany({ where: { transaction: { createdAt: { gte: thisMonthStart, lte: thisMonthEnd }, status: 'COMPLETED' } }, select: { subtotal: true, transaction: { select: { createdAt: true } } } }),
    prisma.expense.findMany({ where: { date: { gte: thisMonthStart, lte: thisMonthEnd } }, select: { total: true, date: true } }),
    prisma.orderItem.findMany({ where: { transaction: { createdAt: { gte: yearStart }, status: 'COMPLETED' } }, select: { subtotal: true, transaction: { select: { createdAt: true } } } }),
