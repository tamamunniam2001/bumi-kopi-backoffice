...
function SectionTitle({ title, sub }) {
  return (
    <div style={{ marginBottom: '20px' }}>
      <h2 style={{ fontSize: '15px', fontWeight: '700', color: '#0F1729', letterSpacing: '-0.2px' }}>{title}</h2>
...
const DualTooltip = ({ active, payload, label }) => {
  ...
  return (
    <div style={{ background: '#0F1729', border: 'none', borderRadius: '12px', padding: '12px 16px', boxShadow: '0 8px 32px rgba(0,0,0,0.25)' }}>
...
function CategoryTable({ title, sub, data, colorStart }) {
  ...
  return (
    <div style={{ background: '#fff', borderRadius: '20px', padding: '24px', border: '1px solid #E2E8F8', boxShadow: '0 2px 16px rgba(15,23,41,0.06)', height: '100%' }}>
      ...
      <main className="main">
        ...
        <div className="content">
          {!data ? (
            ...
            <>
              {/* ── 1. Chart Pendapatan vs Pengeluaran Bulan Ini ── */}
              <div style={{ background: '#fff', borderRadius: '20px', padding: '28px', border: '1px solid #E2E8F8', boxShadow: '0 2px 16px rgba(15,23,41,0.06)', marginBottom: '20px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '24px' }}>
                  <div>
                    ...
                    <p style={{ fontSize: '12px', color: '#8896B3', marginTop: '3px' }}>Harian — {data.currentMonth} {new Date().getFullYear()}</p>
                  </div>
                  <div style={{ display: 'flex', gap: '20px' }}>
                    {[
                      { color: '#6366F1', label: 'Pendapatan', value: data.month.revenue },
                      { color: '#EF4444', label: 'Pengeluaran', value: data.dailyChart.reduce((s, d) => s + d.expense, 0) },
                    ].map(s => (
                ...
                <ResponsiveContainer width="100%" height={220}>
                  <AreaChart data={data.dailyChart} margin={{ top: 5, right: 5, bottom: 0, left: 0 }}>
                    <defs>
              ...
              <div style={{ background: '#fff', borderRadius: '20px', padding: '28px', border: '1px solid #E2E8F8', boxShadow: '0 2px 16px rgba(15,23,41,0.06)', marginBottom: '20px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '24px' }}>
                  <div>
                    ...
                    <p style={{ fontSize: '12px', color: '#8896B3', marginTop: '3px' }}>Perbandingan tiap bulan — {new Date().getFullYear()}</p>
                  </div>
                ...
                <ResponsiveContainer width="100%" height={220}>
                  <BarChart data={data.monthlyChart} margin={{ top: 5, right: 5, bottom: 0, left: 0 }} barGap={4}>
                    <CartesianGrid strokeDasharray="3 3" stroke="#F0F4FF" vertical={false} />
                    <XAxis dataKey="month" tick={{ fontSize: 11, fill: '#8896B3' }} axisLine={false} tickLine={false} />
                    <YAxis tickFormatter={fmtShort} tick={{ fontSize: 11, fill: '#8896B3' }} axisLine={false} tickLine={false} width={48} />
                    <Tooltip content={<DualTooltip />} cursor={{ fill: 'rgba(99,102,241,0.04)', radius: 8 }} />
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
                <CategoryTable
                  ...
                  sub={`Bulan ${data.currentMonth} ${new Date().getFullYear()}`}
