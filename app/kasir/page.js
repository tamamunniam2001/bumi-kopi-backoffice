    ...
    <div className="page">
      ...
      <main className="main" style={{ overflow: 'hidden' }}>
        ...
        <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
                  ...
                  {filtered.map((p) => {
                    ...
                    return (
                      <div key={p.id} onClick={() => addToCart(p)}
                        style={{ background: '#fff', borderRadius: '12px', border: `2px solid ${qty > 0 ? 'var(--accent)' : 'var(--border)'}`, cursor: outOfStock ? 'not-allowed' : 'pointer', opacity: outOfStock ? 0.5 : 1, overflow: 'hidden', transition: 'border-color 0.15s', boxShadow: '0 1px 4px rgba(13,21,38,0.06)' }}>
          ...
          {/* Kanan: Keranjang */}
          <div style={{ width: '320px', background: '#fff', borderLeft: '1px solid var(--border)', display: 'flex', flexDirection: 'column', flexShrink: 0 }}>
      ...
      {checkoutOpen && (
        <CheckoutModal
          ...
          onSuccess={(tx) => { clearCart(); setCheckoutOpen(false); load() }}
        />
      )}
    </div>
  )
...
function CheckoutModal({ cart, total, onClose, onSuccess }) {
  ...
  const paid = Number(payment.replace(/\./g, '')) || 0
  ...
  async function checkout() {
    ...
    setLoading(true)
    try {
      const items = cart.map((i) => ({ productId: i.product.id.startsWith('manual_') ? undefined : i.product.id, name: i.product.id.startsWith('manual_') ? i.product.name : undefined, qty: i.qty, price: i.product.price })).map((i) => { const o = {}; if (i.productId) o.productId = i.productId; if (i.name) o.name = i.name; o.qty = i.qty; o.price = i.price; return o })
